...
export default function ActiveFlight() {
    ...
    const presetRouteMatch = useMemo(() => {
        ...
        return PRESET_ROUTES.find((route) => route.origin.toUpperCase() === origin && route.destination.toUpperCase() === destination) ?? null;
    }, [currentFlight]);
    ...
    const telemetry = useMemo(() => {
        ...
        const altitudeFt = Math.max(30000, Math.round((59000 + Math.sin(phaseMinutes / 6) * 180 + Math.cos(phaseMinutes / 11) * 90) / 10) * 10);
